...
\solution{pr:4} Suppose that there exists a nonzero $\vct{x}\geq \zerovct$ such that $\mtx{A}\vct{x}=\sum_{i=1}^d x_i\vct{a}_i=\zerovct$, where the $\vct{a}_i$ are the rows of $\mtx{A}$. Now if there was a $\zerovct\neq \vct{y}\in \R^m$ such that $\mtx{A}^{\trans}\vct{y}>0$, then $\ip{\vct{a}_i}{\vct{y}}>0$ for all $1\leq i\leq d$, so that
\begin{equation*}
 0 =\ip{\vct{0}}{\vct{y}} = \sum_{i=1}^d x_i \ip{\vct{a}_i}{\vct{y}} >0,
\end{equation*}
